...
-- | Process
maxManhattan scanners = maximum [manhattan src dst | src <- points, dst <- points]
    where
        manhattan (Point sx sy sz) (Point dx dy dz) = (abs (sx-dx)) + (abs (sy-dy)) + (abs (sz-dz))
        points = Set.toList $ process scanners

process (scanner:scanners) = Set.insert (Point 0 0 0) $ process' [scanner] scanners

process' :: [Scanner] -> [Scanner] -> Set.Set Point
process' s@((Scanner sname spoints svectors):todo) scanners = Set.union (Set.fromList $ map (\(o, r, t) -> t) overlaps) (process' (todo++ntodo) nooverlaps)
    where
...
main :: IO ()
main = printf "Day19 Part2: %s" $ show $ maxManhattan $ input "inputs/19"
