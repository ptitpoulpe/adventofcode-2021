...
process ((signal_pattern, digits_outputs):other) =
    (subProcess signal_pattern digits_outputs) + process other
process [] = 0

subProcess :: [Set.Set Char] -> [Set.Set Char] -> Int
subProcess signal_patterns digits_outputs = 
    foldl (\x y -> x * 10 + y) 0 $ map (\x -> fromMaybe 0 $ elemIndex x patterns) digits_outputs
    where
...
main :: IO ()
main = printf "Day08 Part2: %s" $ show $ process $ input "inputs/08"